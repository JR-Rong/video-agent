...
def _external_text_score(*, cand_page_url: str, cand_download_url: str, query: str, required_terms: list[str], banned_terms: list[str]) -> tuple[float, list[str], int]:
    reasons: list[str] = []
    ...
    if hits:
        return 0.0, [f"banned_terms_hit={hits[:6]}"], 0
    ...
    return score, reasons, len(req_hits)
...
def _required_terms_from_search_keywords(search_keywords_en: str) -> list[str]:
    ...
    for t in toks:
        if len(t) < 3:
            continue
        if t in ("ancient", "history", "traditional", "architecture", "realism"):
            continue
        out.append(t)
    return out[:12]
...
def render_media_video(
    *,
    ...
    if era_guard_enabled:
        if provider == "qwen_compat":
            if qwen_api_key and qwen_base_url:
                primary_judge = OpenAICompatVisionJudge(base_url=qwen_base_url, api_key=qwen_api_key, model=qwen_model, timeout_sec=judge_timeout)
            else:
                log.warning("era_guard primary=qwen_compat but missing QWEN_API_KEY or qwen_base_url/QWEN_BASE_URL")
        elif provider == "openai_compat":
            if openai_api_key and openai_base_url:
                primary_judge = OpenAICompatVisionJudge(base_url=openai_base_url, api_key=openai_api_key, model=openai_model, timeout_sec=judge_timeout)
            else:
        ...
        if fallback_provider == "openai_compat":
            if openai_api_key and openai_base_url:
                fallback_judge = OpenAICompatVisionJudge(base_url=openai_base_url, api_key=openai_api_key, model=openai_model, timeout_sec=judge_timeout)
        elif fallback_provider == "qwen_compat":
            if qwen_api_key and qwen_base_url:
                fallback_judge = OpenAICompatVisionJudge(base_url=qwen_base_url, api_key=qwen_api_key, model=qwen_model, timeout_sec=judge_timeout)
    ...
    recent_keys = deque(maxlen=cooldown)

    clip_paths: list[str] = []
    scene_assets: list[dict[str, Any]] = []

    # extract tags from prompts (Region/Era/Culture) if present
        # ----- era_profile (preferred) -----
    region = ""
    era = ""
    period = ""

    required_terms: list[str] = _required_terms_from_search_keywords(search_keywords_en)

    if isinstance(era_profile, dict):
        setting = era_profile.get("setting") or {}
        region = str(setting.get("region") or "").strip()
        era = str(setting.get("era") or "").strip()
        period = str(setting.get("period") or "").strip() or str(setting.get("era") or "").strip()

        sk = era_profile.get("search_keywords_en") or {}
        rt = sk.get("required_terms") or []
        if isinstance(rt, list) and rt:
            required_terms = [str(x).strip().lower() for x in rt if str(x).strip()][:12]

        vr = era_profile.get("visual_rules") or {}
        # if era_profile provides allowed_people_hint, override config default
        if isinstance(vr, dict):
            aph = str(vr.get("allowed_people_hint") or "").strip()
            if aph:
                allowed_people_hint = aph

            # You can also merge banned_objects/banned_styles into banned_terms if you want stricter filtering
            # Here we keep it for vision judge mainly.
        sk_avoid = sk.get("avoid_terms") or []
        if isinstance(sk_avoid, list) and sk_avoid:
            for t in sk_avoid:
                tt = str(t).strip().lower()
                if tt and tt not in banned_terms:
                    banned_terms.append(tt)
    ...
    def era_guard_check(*, image_path: str, scene_id: int, kind: str) -> bool:
        ...
        user_text = render_template(
            tpl,
            {
                ...
                "period": period or (era or "unknown"),
                "allowed_people_hint": allowed_people_hint,
                "hard_block_objects": hard_block_objects,
            },
        )
        if tracer:
            tracer.emit("era_guard_try", scene_id=scene_id, kind=kind, image_path=image_path, provider=provider, fallback_provider=fallback_provider)
        try:
    ...
    for scene, sec in zip(scenes, allocations):
        ...
        # strict history suggestion: key scenes prefer generation (not external). Here kept generic:
        allow_external_for_scene = True
        if strict_mode and importance == "key":
            # 宁缺毋滥：关键镜头不优先外搜
        ...
        if allow_external_for_scene and external_media and bool(external_cfg.get("enabled", False)):
            ...
            for c in vids:
                ...
                score, reasons, req_n = _external_text_score(
        ...
        if allow_external_for_scene and external_media and bool(external_cfg.get("enabled", False)):
            ...
            for c in imgs:
                ...
                img_prev = os.path.join(run_dir, f"extprev_{sid:03d}_{c.provider}_{c.id}.jpg")
                try:
                    external_media.download(cand=c, out_path=img_prev)
                    ok = era_guard_check(image_path=img_prev, scene_id=sid, kind="image")
                    if not ok:
                        chosen_debug.append({"key": key, "score": score, "drop": "era_guard_fail", "reasons": reasons})
                        continue
                except Exception as e:
            ...
            scene_assets.append(
                {
                    "id": sid,
                    "type": "image_motion",
                    "method": "image_library_or_generated",
                    "image": img_path,
                    "clip": clip_path,
                    "seconds": int(sec),
                }
        ...
        # ---------- Step D: generation fallback (mode-specific) ----------
        # images: always generate image
        if media_mode == "images":
            img_path = os.path.join(run_dir, f"scene_{sid:03d}.png")
            images.generate_image(
                prompt=scene.get("image_prompt") or "",
            ...
            scene_assets.append(
                {
                    "id": sid,
                    "type": "image_motion",
                    "method": "generated_image_fallback",
                    "image": img_path,
                    "clip": clip_path,
                    "seconds": int(sec),
                }
        ...
        # mixed: key -> video; normal -> image (your original strategy)
        if media_mode == "mixed":
            if importance == "key":
                if not video_generator:
                    raise RuntimeError("mixed mode needs video_generator for key scenes.")

                # generate a ref image first (cheaper, controllable)
                ...
                scene_assets.append(
                    {
                        "id": sid,
                        "type": "video",
                        "method": "generated_video_key_fallback",
                        "raw_clip": raw_clip,
                        "clip": norm_clip,
                        "seconds": int(sec),
                        "ref_image": ref_img,
                ...
                scene_assets.append(
                    {
                        "id": sid,
                        "type": "image_motion",
                        "method": "generated_image_normal_fallback",
                        "image": img_path,
                        "clip": clip_path,
                        "seconds": int(sec),
                    }
        ...
        # videos mode: always generate video (image2video)
        if media_mode == "videos":
            if not video_generator:
            ...
            scene_assets.append(
                {
                    "id": sid,
                    "type": "video",
                    "method": "generated_video_fallback",
                    "raw_clip": raw_clip,
                    "clip": norm_clip,
                    "seconds": int(sec),
                    "ref_image": ref_img,
